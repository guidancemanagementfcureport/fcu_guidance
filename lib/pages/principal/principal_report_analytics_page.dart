...
class _PrincipalReportAnalyticsPageState extends State<PrincipalReportAnalyticsPage> {
  ...
  bool _matchesStudentLevelForCurrentRole(String? level, UserRole? currentRole) {
    if (level == null) return false;
    final normalized = level.toLowerCase();
    if (currentRole == UserRole.principal ||
        currentRole == UserRole.assistantPrincipal) {
      return normalized == 'junior_high' || normalized == 'senior_high';
    }
  ...
  List<Map<String, dynamic>> _applyAcademicFilters(
    List<Map<String, dynamic>> reports,
    UserRole? currentRole,
  ) {
    return reports.where((report) {
      final student = report['student'] as Map<String, dynamic>?;
      final level = (student?['student_level'] as String?)?.toLowerCase();
      if (!_matchesStudentLevelForCurrentRole(level, currentRole)) return false;
      if (!_isCounselorForwardedReport(report)) return false;

      if (currentRole == UserRole.principal ||
          currentRole == UserRole.assistantPrincipal) {
        if (_selectedStudentLevel != null && _selectedStudentLevel!.isNotEmpty) {
          if (level != _selectedStudentLevel) return false;
        }
        return true;
      }

      final course = (student?['course'] as String?)?.trim().toLowerCase();
      final yearLevel =
          (student?['year_level'] as String?)?.trim().toLowerCase();
      ...
      if (_selectedYearLevel != null &&
          ...
          yearLevel != _selectedYearLevel!.toLowerCase()) {
        return false;
      }
      return true;
    }).toList();
  ...
  Future<void> _loadData() async {
    final currentRole = Provider.of<AuthProvider>(context, listen: false).currentUser?.role;
    setState(() => _isLoading = true);

    try {
      final reports = await _supabase.getAllReportsForAdmin();

      _allReports = _applyAcademicFilters(reports, currentRole);

      _settledReports =
          _allReports.where((r) {
            final status = (r['status'] as String?)?.toLowerCase() ?? '';
            final counselorNote =
                (r['counselor_note'] as String?)?.toLowerCase() ?? '';
            final isForwardedSettledCopy =
                counselorNote.contains('forwarded copy from settled counseling record');
            return status == 'settled' ||
                status == 'completed' ||
                isForwardedSettledCopy;
          }).length;
      ...
      if (_searchQuery.isNotEmpty) {
        ...
        _filteredReports =
            _allReports.where((report) {
              final student = report['student'] as Map<String, dynamic>?;
              final currentRole = Provider.of<AuthProvider>(
                context,
                listen: false,
              ).currentUser?.role;
              if (_applyAcademicFilters([report], currentRole).isEmpty) {
                return false;
              }
        ...
        _filteredReports = _applyAcademicFilters(reports, currentRole);
      }
                    ...
                    builder: (context) {
                      ...
                      if (currentRole == UserRole.principal ||
                        ...
                        return _buildModernDropdown(
                          ...
                          onChanged: (v) {
                            setState(() => _selectedStudentLevel = v);
                            _applyFilters();
                          },
    ...
    return Column(
      children: [
        Row(
          ...
          children: [
            Text(
              'Settled Cases (${_filteredReports.length})',
              style: const TextStyle(
                fontSize: 20,
                fontWeight: FontWeight.bold,
                color: AppTheme.deepBlue,
